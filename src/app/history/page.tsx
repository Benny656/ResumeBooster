...
export default function History() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-12 w-full pt-8 flex flex-col gap-10">
      
      {/* Header */}
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-3xl font-heading font-bold mb-2">Analysis History</h1>
          <p className="opacity-60 text-sm">Review your past optimizations and track your progress.</p>
        </div>
      </section>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <button className="bg-white border border-black/10 px-4 py-3 rounded-xl font-medium text-sm flex items-center justify-center gap-2 hover:bg-black/5 transition-colors">
        ...
        {loading ? (
          ...
          history.filter(item => 
            ...
            <motion.div 
              ...
              key={item._id} 
              className="glass-card p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 group hover:bg-white/80 transition-colors"
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => handleDelete(item._id)} className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-red-50 hover:text-red-600 transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100 text-[var(--color-brand-black)]">
                  <Trash2 size={18} />
                </button>
              </div>
            </motion.div>
